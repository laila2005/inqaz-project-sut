...
**Answer:** Yes. During a visual data audit, we discovered that the original Kaggle dataset had mislabeled images mixed between the two folders. Some crash images appeared in the normal folder and vice versa. We wrote a diagnostic script that ran the model predictions on samples from each folder and measured the average confidence. This revealed that the model was "confused" — it couldn't clearly separate the two classes because the labels were noisy. We corrected this by physically reorganizing the folders to ensure clean labels, then retrained the model. This dramatically improved accuracy and is a textbook example of why **data validation is as important as model architecture** in machine learning.mes.
